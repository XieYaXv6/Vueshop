...
const shopModel = models.getModel('shop')
const _filter = {'pwd': 0, '__v': 0} // 查询时过滤掉 指定过滤的属性
  ...
  UserModel.findOne({name}, function (err, user) {
    if (user) {
      ...
      userModel.save(function (err, user) {
        ...
        // res.cookie('userid', user._id, {maxAge: 1000*60*60*24*7})//存活时间
        req.session.userid = user._id
        const data = {_id: user._id, name: user.name}//不返回密码
        // 3.2. 返回数据(新的user)
...
/*
根据sesion中的userid, 查询对应的user，然后自动登录 实现免登录
 */
...
router.get('/index_category',async function (req, res) {
  ...
  //   res.send({code: 0, data})
  // ,300})
  //categoryModel.find(function (err, categorys) { // 如果有匹配返回的是一个[user, user..], 如果没有一个匹配的返回[]

  // UserModel.find(function (err, user) {
  //   const data =('find()', err, user)
  //   //const data =find()
...
router.get('/search_shops', function (req, res) {
  ...
  shopModel.find({name:{$regex:keyword}},function(err,shop){
    res.send({code:0,shop})
  })
})
