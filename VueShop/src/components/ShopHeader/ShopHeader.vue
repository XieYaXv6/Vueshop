<template>
  <div class="shop-header">
    ...
    <div class="shop-header-discounts" v-if="info.supports" @click="toggleSupportShow"> 
        <!-- v-if="info.supports"避免在没有数据时[初始值]解析，之前解析没出错是因为其他的是二级表达式，而这个supports是三级-->
      <div class="discounts-left">
        <div class="activity" :class="supportClasses[info.supports[0].type]">  <!-- 用data中定义的颜色属性进行颜色渲染 -->
          <span class="content-tag">
...
<script>
  ...
  export default {
    data () {
      return {
        supportClasses: ['activity-green', 'activity-red', 'activity-orange'],//优惠活动的颜色属性
        shopShow: false,
        supportShow: false
      }
    },
